...
public:
    ...
    template<class T>
    void operator()(T const&, std::string_view);

    template<class T>
    void operator()(T const&, std::string_view, SimpleWriterTag);

    bool containsOnlyNamespaces(NamespaceInfo const& I) const;    

#define INFO_PASCAL(Type) void write##Type(Type##Info const&, std::string_view);
#include <mrdocs/Metadata/InfoNodes.inc>

#define INFO_PASCAL(Type) void write##Type(Type##Info const&, std::string_view, SimpleWriterTag);
#include <mrdocs/Metadata/InfoNodes.inc>

    template<class T>
    void writeClassLike(T const& I, std::string_view);
    template<class T>
    void writeClassLike(T const& I, std::string_view, SimpleWriterTag);
